...
#
# def search_service_codes(query: str, top_k: int = 10, min_similarity: float = 0.5, fallback: bool = True) -> list[dict]:
#     """
#     Returns top-k service code candidates for a SOAP note query.
#     Each entry includes code, description, and similarity score.
#     If no candidate exceeds min_similarity, optionally return the top candidate as fallback.
#     """
#     # Generate embedding
...
#         if idx < 0:
#             continue
#         similarity = 1 - (dist ** 2) / 2  # cosine similarity
...
#         })
#
#     # Fallback: return top candidate if no results
...
#     """Return {code: description} for the given list of codes."""
#     code_set = set(codes)
...
def search_service_codes(query: str, top_k: int = 10, min_similarity: float = 0.5, fallback: bool = True) -> List[Dict[str, Any]]:
    """
    Finds and returns top-k service code candidates for a given SOAP note query.

    Args:
        query: The SOAP note text to search for.
        top_k: The number of top candidates to retrieve from the FAISS index.
        min_similarity: The minimum cosine similarity score a candidate must have to be returned.
        fallback: If True, returns the single top candidate even if it falls below the min_similarity.

    Returns:
        A list of dictionaries, where each dictionary contains the 'code', 'description',
        and 'similarity' score of a matching service code.
    """
    # Generate a normalized embedding for the query
    embedding = normalize_vector(model.encode([query], convert_to_numpy=True)[0])

    # Search the FAISS index for the nearest neighbors
    # FAISS returns squared L2 distance (D) and indices (I)
    D, I = index.search(np.array([embedding]), k=top_k)
    ...
    # Fallback logic: if no candidates meet the similarity threshold, return the single best one
    if not results and fallback and len(I[0]) > 0 and I[0][0] >= 0:
        idx = I[0][0]
        code, desc = all_codes[idx]
        similarity = 1 - (D[0][0] ** 2) / 2
...
def get_service_code_descriptions(codes: List[str]) -> Dict[str, str]:
    ...
    Args:
        codes: A list of service code strings.

    Returns:
        A dictionary mapping each service code to its description.
    """
    code_set = set(codes)
    return {code: desc for code, desc in all_codes if code in code_set}

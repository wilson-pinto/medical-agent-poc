...
async def check_referral_required_node(
    ...
    """
    Node to determine if a referral check is required.
    Sets 'requires_referral_check' flag based on predicted service codes or SOAP content.
    """
    updates = {}

    # Check if any predicted service code might need referral
...
async def execute_referral_node(
    ...
    stages.append(StageEvent(
        ...
        data={
            "reasoning": reasoning,
            "applied_rule": updates.get("referral_rule_applied"),
            "triggered_by": triggered_by
        }
    ))
    updates["stages"] = stages
